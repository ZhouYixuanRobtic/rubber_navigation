...
BaseController::BaseController(const std::string &serial_addr,
                               unsigned int baudrate,
                               std::string base_foot_print,
                               std::string odom_frame, bool publish_tf)
  ...
  if (serialManager->openSerial()) {
    ...
    wheel_status_pub =
        nh_.advertise<rubber_navigation::WheelStatus>("/wheel_status", 100);
    odom_raw_pub = nh_.advertise<nav_msgs::Odometry>("/odom_raw", 100);
    cmd_vel_sub =
        nh_.subscribe("joy_vel", 100, &BaseController::joy_velCallback, this);
    joy_vel_sub =
        nh_.subscribe("cmd_vel", 100, &BaseController::cmd_velCallback, this);
    init_send_msgs();

    read_timer_ = nh_.createTimer(ros::Duration(1.0 / TIMER_SPAN_RATE_),
                                  &BaseController::readTimerCallback, this);
    read_timer_.start();
    send_timer_ = nh_.createTimer(ros::Duration(1.0 / TIMER_SPAN_RATE_),
                                  &BaseController::sendTimerCallback, this);
    send_timer_.start();
    odom_publish_timer_ =
        nh_.createTimer(ros::Duration(1.0 / ODOM_TIMER_SPAN_RATE_),
                        &BaseController::odom_publish_timer_callback, this);
    odom_publish_timer_.start();
    ROS_INFO_STREAM("BASE READY!!!");
  } else
    ROS_ERROR_STREAM("Can't open "
                     << "SERIAL " << serial_addr << std::endl);
}
BaseController::~BaseController() { delete serialManager; }
...
unsigned char BaseController::xor_msgs(unsigned char *msg) {
  ...
  for (int i = 2; i < 7; i++) check = check ^ msg[i];
  return check;
}
void BaseController::init_send_msgs() { get_pos[7] = xor_msgs(get_pos); }
void BaseController::odom_parsing() {
  static int right_encoder_pre{ENCODER_.right_encoder},
  ...
  if (ENCODER_.right_encoder == 0 && ENCODER_.left_encoder == 0 &&
      right_encoder_pre != 0 && left_encoder_pre != 0) {
    // we hope to keep the same value of last time
    // so just update value and return;
  ...
  right_distance = M_PI * BASE_MODEL_.Wheel_Diameter * right_delta /
                   BASE_MODEL_.Encoder_to_Distance;  // mm
  left_distance = M_PI * BASE_MODEL_.Wheel_Diameter * left_delta /
                  BASE_MODEL_.Encoder_to_Distance;  // mm
  ...
  if (left_delta == right_delta) {
    ...
    global_x = global_x + vertical_robot * cos(global_theta) -
               horizontal_robot * sin(global_theta);  // m
    global_y = global_y + horizontal_robot * cos(global_theta) +
               vertical_robot * sin(global_theta);  // m
    ...
    turning_radius =
        (BASE_MODEL_.Wheel_Base * (right_distance + left_distance)) /
        (2.0 * (right_distance - left_distance));  // m
      ...
      case 0x20: {
        ...
        if (message_[3] == 0x01) {
          if (message_[4] == 0x01) {
            ...
            std::cout << "Debug BaseController leftend" << std::endl;

            knife_left_end = true;
          } else {
            ros::param::set("/visual_servo/knifeLeftMoveEnd", 0.0);
            std::cout << "Debug BaseController notleftend" << std::endl;
          ...
          if (message_[4] == 0x01) {
            ...
            std::cout << "Debug BaseController rightend" << std::endl;
            knife_right_end = true;
          } else {
            ros::param::set("/visual_servo/knifeRightMoveEnd", 0.0);
            std::cout << "Debug BaseController notrightend" << std::endl;
...
void BaseController::readTimerCallback(const ros::TimerEvent &e) {
  ...
  if (ENCODER_.interval != 0) {
    if (ENCODER_.interval > 20.0 / TIMER_SPAN_RATE_ ||
        ENCODER_.interval < 0.0) {
      if (!ENCODER_.encoderWrong) encoder_stop = ros::Time::now();
...
void BaseController::odom_publish_timer_callback(const ros::TimerEvent &e) {
  ...
  geometry_msgs::Quaternion odom_quat =
      tf::createQuaternionMsgFromYaw(global_theta);
  ...
  if (publish_tf_) broad_caster.sendTransform(odom_trans);
}
void BaseController::cmd_velCallback(const geometry_msgs::TwistConstPtr &msg) {
  ...
  if (linear_velocity == 0) {
    ...
    right_vel = linear_velocity +
                angular_velocity * BASE_MODEL_.Wheel_Base / 2000.0;  // m
    left_vel = linear_velocity -
               angular_velocity * BASE_MODEL_.Wheel_Base / 2000.0;  // m
  }
  user_cmd_vel.cmd_right =
      right_vel * 60.0 / (M_PI * BASE_MODEL_.Wheel_Diameter / 1000.0);
  user_cmd_vel.cmd_left =
      left_vel * 60.0 / (M_PI * BASE_MODEL_.Wheel_Diameter / 1000.0);
...
void BaseController::joy_velCallback(const geometry_msgs::TwistConstPtr &msg) {
  ...
  if (cmd_vel_received_ &&
      ros::Time::now() - cmd_vel_watch_ > ros::Duration(0.5))
...
void BaseController::sendCommand() {
  ...
  switch (USER_COMMAND_.user_command) {
    ...
    case CLOCK_GO:
      ...
      std::cout << "get switch 01 " << std::endl;
      usleep(33333);
      if (!knife_left_end) {
        serialManager->send(clockGo, COMMAND_SIZE);
        std::cout << "clock go " << std::endl;
      }
      break;
    case ANTI_CLOCK_GO:
      get_switch[3] = 0x02;
      serialManager->send(get_switch, COMMAND_SIZE);
      std::cout << "get switch 02" << std::endl;
      usleep(33333);
      if (!knife_right_end) {
        serialManager->send(antiClockGo, COMMAND_SIZE);
        std::cout << "anti clock go " << std::endl;
      }
      break;
    case KNIFE_ON:
    ...
    case SINGLE_CLOCK_GO:
      ...
      serialManager->send(get_switch, COMMAND_SIZE);
      usleep(33333);
      if (!knife_left_end) serialManager->send(singleClockGo, COMMAND_SIZE);
      break;
    ...
    case GET_SWITCH:
      if (USER_COMMAND_.parameter == 1) {
        get_switch[3] = 0x01;
        std::cout << "debug base_controller: get_switch" << get_switch
                  << std::endl;
      } else {
        get_switch[3] = 0x02;
        std::cout << "debug base_controller: get_switch" << get_switch
